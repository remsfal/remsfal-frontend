...
describe('ContractorService (MSW with http)', () => {
  ...
  it('should get a list of issues', async () => {
    ...
    const issueList = await contractorService.getIssues();
  ...
  it('should get a single issue', async () => {
    ...
    const issue = await contractorService.getIssue(issueId);
  ...
  it('should handle issue retrieval error', async () => {
    ...
    await expect(contractorService.getIssue('non-existing')).rejects.toThrow();
  });
});
