...
import { getRelativeTime, getStatusColor, getStatusIcon, resolveProjectName }
  from '@/features/manager/activityFeeds/composables/useActivityFeedHelpers';
import type { ProjectItem } from '@/services/ProjectService';

describe('useActivityFeedHelpers', () => {
  let mockDate: Date;
  ...
  describe('resolveProjectName', () => {
    ...
    it('returns the matching project name', () => {
      expect(resolveProjectName('proj-1', projectList)).toBe('Project 1');
    });

    it('falls back to the projectId when no match is found', () => {
      expect(resolveProjectName('proj-unknown', projectList)).toBe('proj-unknown');
    });

    it('falls back to the projectId when the project list is empty', () => {
      expect(resolveProjectName('proj-1', [])).toBe('proj-1');
    });
  });
});
