...
import type { CustomFilter, ProjectOption } from '@/features/manager/activityFeeds/components/ActivityFeedSidebar.vue';
...
describe('ActivityFeedSidebar', () => {
  ...
  const mountWithProps = (overrides?: Partial<{
    ...
    wrapper = mount(ActivityFeedSidebar, {
      props: {
  ...
  it('treats filter query parts with unknown keys as always matching', () => {
    mountWithProps({
      ...
      entries: [createMockActivityFeedEntry({ id: 'x1' })],
      projectOptions: [],
    });
