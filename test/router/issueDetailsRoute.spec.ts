...
describe('Router - Issue Details Route', () => {
  ...
  describe('Route Configuration', () => {
    ...
    it('should resolve to ProjectIssueView component', async () => {
      ...
      // The component is lazy-loaded, so we check the route exists
  ...
  describe('Route Navigation', () => {
    ...
    it('should update route params on navigation', async () => {
      await router.push({
        name: 'IssueDetails',
        params: { projectId: 'proj-1', issueId: 'issue-1' },
      });
      ...
      await router.push({
        name: 'IssueDetails',
        params: { projectId: 'proj-1', issueId: 'issue-2' },
      });
  ...
  describe('Route Props Function', () => {
    it('should extract props from route params', () => {
      const route = router.resolve({
        name: 'IssueDetails',
        params: { projectId: 'test-proj', issueId: 'test-issue' },
      });
    ...
    it('should handle missing params gracefully', () => {
      // Try to create route without required params
      const route = router.resolve('/projects//issues/');
      
      // Router should still create a route (might not match IssueDetails)
  ...
  describe('Route Hierarchy', () => {
    it('should be a child of manager routes', () => {
      ...
      // Should have parent routes (AppLayout, ManagerMenu, etc.)
    ...
    it('should match nested route pattern', () => {
      ...
      // Path should contain /projects/:projectId/issues/:issueId pattern
  ...
  describe('Router Integration', () => {
    it('should work with router.resolve', () => {
      const resolved = router.resolve({
        name: 'IssueDetails',
        params: { projectId: 'abc', issueId: '123' },
      });
    ...
    it('should generate correct href for router-link', () => {
      const resolved = router.resolve({
        name: 'IssueDetails',
        params: { projectId: 'my-project', issueId: 'issue-789' },
      });
    ...
    it('should be accessible from other routes', async () => {
      // Start from different route
      await router.push({ name: 'LandingPage' });
      expect(router.currentRoute.value.name).toBe('LandingPage');
      
      // Navigate to IssueDetails
      await router.push({
        name: 'IssueDetails',
        params: { projectId: 'test', issueId: 'test' },
      });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle encoded URL characters', () => {
      const route = router.resolve({
        name: 'IssueDetails',
        params: { projectId: 'proj%20space', issueId: 'issue%20space' },
      });
      
      // Vue Router decodes params automatically
  ...
  describe('Route Meta and Guards', () => {
    it('should have parent route with beforeEnter guard', () => {
      ...
      // Parent route should have beforeEnter to load project
    ...
    it('should inherit layout from parent route', () => {
      ...
      // Should have components from parent (topbar, sidebar)
