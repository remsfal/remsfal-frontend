describe('NewRentalAgreementDialog E2E Tests', () => {
  ...
  const openAddKeyDialog = () => {
    cy.contains('button', 'Schlüssel zum Mietertrag hinzufügen').click();
  };
  const keyDialog = () => cy.contains('[role="dialog"]', 'Schlüssel hinzufügen');

  const selectIssuedAtToday = () => {
    cy.get('input#issuedAt').click();
    cy.get('.p-datepicker-panel td[data-p-today="true"] span').first().click();
  };

  it('adds a key with amount, description and issue date', () => {
    goToStep2();
    openAddKeyDialog();
    ...
    keyDialog().within(() => {
      cy.contains('button', 'Hinzufügen').should('not.be.disabled').click();
    });

    cy.get('[role="dialog"]').should('have.length', 1);
    cy.contains('Anzahl der Schlüssel: 2').should('be.visible');
    cy.contains('Haustürschlüssel').should('be.visible');
  });

  it('removes a previously added key', () => {
    goToStep2();
    openAddKeyDialog();
    keyDialog().within(() => {
      cy.get('input[name="amountOfKeys"]').type('1');
      cy.get('input[name="keyDescription"]').type('Kellerschlüssel');
    });
    selectIssuedAtToday();
    keyDialog().within(() => {
    ...
    cy.contains('Kellerschlüssel').should('be.visible');

    cy.get('[aria-label="Schlüssel entfernen"]').click();

    cy.contains('Kellerschlüssel').should('not.exist');
  });

  it('allows adding more than one key', () => {
    goToStep2();
    openAddKeyDialog();
    keyDialog().within(() => {
      cy.get('input[name="amountOfKeys"]').type('2');
    ...
    keyDialog().within(() => {
      cy.get('input[name="amountOfKeys"]').type('1');
      cy.get('input[name="keyDescription"]').type('Briefkastenschlüssel');
    });
    selectIssuedAtToday();
    keyDialog().within(() => {
      cy.contains('button', 'Hinzufügen').click();
    ...
    cy.contains('Haustürschlüssel').should('be.visible');
    cy.contains('Briefkastenschlüssel').should('be.visible');
    cy.get('[aria-label="Schlüssel entfernen"]').should('have.length', 2);
  });
});
