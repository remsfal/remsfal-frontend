...
const fetchTimelines = async () => {
  ...
  error.value = false;
  try {
    const timelineResponse = await issueTimelineService.getTimelineEntries(props.issueId);
...
const submitMessage = async () => {
  ...
  try {
    await issueTimelineService.createTimelineEntryWithAttachments(props.issueId, {
  ...
  <BaseCard>
    ...
    <template #content>
      <CardSkeletonRows
        ...
        :rows="3"
        rowHeight="3rem"
        data-testid="issue-timeline-loading"
      />

      <Message v-else-if="error" severity="error" :closable="false" data-testid="issue-timeline-error">
      ...
      <div class="mb-4 flex flex-col gap-2">
        <Textarea
          id="issue-timeline-message"
          v-model="messageText"
          data-testid="issue-timeline-message-input"
          rows="3"
        ...
        <div class="flex justify-end">
          <Button
            data-testid="issue-timeline-message-submit"
