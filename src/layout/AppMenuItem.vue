...
const itemClick = (event: Event, item: any) => {
  ...
  isActiveMenu.value = foundItemKey === itemKey.value || foundItemKey!.startsWith(itemKey.value + '-');
};
...
<template>
  <li :class="{ 'layout-root-menuitem': root, 'active-menuitem': isActiveMenu }">
    ...
    <a v-if="(!item.to || item.items) && item.visible !== false" @click="itemClick($event, item)" :class="item.class"
    ...
    <Transition v-if="item.items && item.visible !== false" name="layout-submenu">
      <ul v-show="root ? true : isActiveMenu" class="layout-submenu">
        <app-menu-item v-for="(child, i) in item.items" :key="child.label" :index="i" :item="child" :parentItemKey="itemKey"
